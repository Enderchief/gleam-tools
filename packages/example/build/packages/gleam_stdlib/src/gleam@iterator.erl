...
-type action(BOY) :: stop | {continue, BOY, fun(() -> action(BOY))}.

-opaque iterator(BOZ) :: {iterator, fun(() -> action(BOZ))}.

-type step(BPA, BPB) :: {next, BPA, BPB} | done.

-type chunk(BPC, BPD) :: {another_by,
        list(BPC),
        BPD,
        BPC,
        fun(() -> action(BPC))} |
...
-type sized_chunk(BPE) :: {another, list(BPE), fun(() -> action(BPE))} |
    {last, list(BPE)} |
    no_more.
...
-spec do_unfold(BPH, fun((BPH) -> step(BPI, BPH))) -> fun(() -> action(BPI)).
do_unfold(Initial, F) ->
...
-spec unfold(BPM, fun((BPM) -> step(BPN, BPM))) -> iterator(BPN).
unfold(Initial, F) ->
    _pipe = Initial,
    _pipe@1 = do_unfold(_pipe, F),
    {iterator, _pipe@1}.

-spec repeatedly(fun(() -> BPR)) -> iterator(BPR).
repeatedly(F) ->
    unfold(nil, fun(_) -> {next, F(), nil} end).

-spec repeat(BPT) -> iterator(BPT).
repeat(X) ->
    repeatedly(fun() -> X end).

-spec from_list(list(BPV)) -> iterator(BPV).
from_list(List) ->
...
-spec transform(iterator(BQF), BQH, fun((BQH, BQF) -> step(BQI, BQH))) -> iterator(BQI).
transform(Iterator, Initial, F) ->
    _pipe = do_transform(erlang:element(2, Iterator), Initial, F),
    {iterator, _pipe}.

-spec do_fold(fun(() -> action(BQM)), fun((BQO, BQM) -> BQO), BQO) -> BQO.
...
-spec fold(iterator(BQP), BQR, fun((BQR, BQP) -> BQR)) -> BQR.
...
-spec to_list(iterator(BQU)) -> list(BQU).
to_list(Iterator) ->
    _pipe = Iterator,
    _pipe@1 = fold(_pipe, [], fun(Acc, E) -> [E | Acc] end),
    gleam@list:reverse(_pipe@1).

-spec step(iterator(BQX)) -> step(BQX, iterator(BQX)).
step(Iterator) ->
...
-spec do_take(fun(() -> action(BRC)), integer()) -> fun(() -> action(BRC)).
...
-spec take(iterator(BRF), integer()) -> iterator(BRF).
take(Iterator, Desired) ->
    _pipe = erlang:element(2, Iterator),
    _pipe@1 = do_take(_pipe, Desired),
    {iterator, _pipe@1}.

-spec do_drop(fun(() -> action(BRI)), integer()) -> action(BRI).
...
-spec drop(iterator(BRL), integer()) -> iterator(BRL).
drop(Iterator, Desired) ->
    _pipe = fun() -> do_drop(erlang:element(2, Iterator), Desired) end,
    {iterator, _pipe}.

-spec do_map(fun(() -> action(BRO)), fun((BRO) -> BRQ)) -> fun(() -> action(BRQ)).
...
-spec map(iterator(BRS), fun((BRS) -> BRU)) -> iterator(BRU).
map(Iterator, F) ->
    _pipe = erlang:element(2, Iterator),
    _pipe@1 = do_map(_pipe, F),
    {iterator, _pipe@1}.

-spec do_append(fun(() -> action(BRW)), fun(() -> action(BRW))) -> action(BRW).
...
-spec append(iterator(BSA), iterator(BSA)) -> iterator(BSA).
append(First, Second) ->
    _pipe = fun() ->
...
-spec do_flatten(fun(() -> action(iterator(BSE)))) -> action(BSE).
...
-spec flatten(iterator(iterator(BSI))) -> iterator(BSI).
flatten(Iterator) ->
    _pipe = fun() -> do_flatten(erlang:element(2, Iterator)) end,
    {iterator, _pipe}.

-spec flat_map(iterator(BSM), fun((BSM) -> iterator(BSO))) -> iterator(BSO).
flat_map(Iterator, F) ->
    _pipe = Iterator,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).

-spec do_filter(fun(() -> action(BSR)), fun((BSR) -> boolean())) -> action(BSR).
...
-spec filter(iterator(BSU), fun((BSU) -> boolean())) -> iterator(BSU).
filter(Iterator, Predicate) ->
    _pipe = fun() -> do_filter(erlang:element(2, Iterator), Predicate) end,
    {iterator, _pipe}.

-spec cycle(iterator(BSX)) -> iterator(BSX).
cycle(Iterator) ->
    _pipe = repeat(Iterator),
    flatten(_pipe).

-spec do_find(fun(() -> action(BTB)), fun((BTB) -> boolean())) -> {ok, BTB} |
    {error, nil}.
...
-spec find(iterator(BTF), fun((BTF) -> boolean())) -> {ok, BTF} | {error, nil}.
find(Haystack, Is_desired) ->
    _pipe = erlang:element(2, Haystack),
    do_find(_pipe, Is_desired).

-spec do_index(fun(() -> action(BTJ)), integer()) -> fun(() -> action({integer(),
    BTJ})).
...
-spec index(iterator(BTM)) -> iterator({integer(), BTM}).
index(Iterator) ->
    _pipe = erlang:element(2, Iterator),
    _pipe@1 = do_index(_pipe, 0),
    {iterator, _pipe@1}.

-spec iterate(BTP, fun((BTP) -> BTP)) -> iterator(BTP).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-spec do_take_while(fun(() -> action(BTR)), fun((BTR) -> boolean())) -> fun(() -> action(BTR)).
...
-spec take_while(iterator(BTU), fun((BTU) -> boolean())) -> iterator(BTU).
take_while(Iterator, Predicate) ->
    _pipe = erlang:element(2, Iterator),
    _pipe@1 = do_take_while(_pipe, Predicate),
    {iterator, _pipe@1}.

-spec do_drop_while(fun(() -> action(BTX)), fun((BTX) -> boolean())) -> action(BTX).
...
-spec drop_while(iterator(BUA), fun((BUA) -> boolean())) -> iterator(BUA).
drop_while(Iterator, Predicate) ->
    _pipe = fun() -> do_drop_while(erlang:element(2, Iterator), Predicate) end,
    {iterator, _pipe}.

-spec do_scan(fun(() -> action(BUD)), fun((BUF, BUD) -> BUF), BUF) -> fun(() -> action(BUF)).
...
-spec scan(iterator(BUH), BUJ, fun((BUJ, BUH) -> BUJ)) -> iterator(BUJ).
scan(Iterator, Initial, F) ->
    _pipe = erlang:element(2, Iterator),
    _pipe@1 = do_scan(_pipe, F, Initial),
    {iterator, _pipe@1}.

-spec do_zip(fun(() -> action(BUL)), fun(() -> action(BUN))) -> fun(() -> action({BUL,
    BUN})).
do_zip(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(iterator(BUQ), iterator(BUS)) -> iterator({BUQ, BUS}).
zip(Left, Right) ->
    _pipe = do_zip(erlang:element(2, Left), erlang:element(2, Right)),
    {iterator, _pipe}.

-spec next_chunk(fun(() -> action(BUV)), fun((BUV) -> BUX), BUX, list(BUV)) -> chunk(BUV, BUX).
...
-spec do_chunk(fun(() -> action(BVB)), fun((BVB) -> BVD), BVD, BVB) -> action(list(BVB)).
...
-spec chunk(iterator(BVG), fun((BVG) -> any())) -> iterator(list(BVG)).
chunk(Iterator, F) ->
...
-spec next_sized_chunk(fun(() -> action(BVL)), integer(), list(BVL)) -> sized_chunk(BVL).
...
-spec do_sized_chunk(fun(() -> action(BVP)), integer()) -> fun(() -> action(list(BVP))).
...
-spec sized_chunk(iterator(BVT), integer()) -> iterator(list(BVT)).
sized_chunk(Iterator, Count) ->
    _pipe = erlang:element(2, Iterator),
    _pipe@1 = do_sized_chunk(_pipe, Count),
    {iterator, _pipe@1}.

-spec do_intersperse(fun(() -> action(BVX)), BVX) -> action(BVX).
...
-spec intersperse(iterator(BWA), BWA) -> iterator(BWA).
...
-spec do_any(fun(() -> action(BWD)), fun((BWD) -> boolean())) -> boolean().
...
-spec any(iterator(BWF), fun((BWF) -> boolean())) -> boolean().
any(Iterator, Predicate) ->
    _pipe = erlang:element(2, Iterator),
    do_any(_pipe, Predicate).

-spec do_all(fun(() -> action(BWH)), fun((BWH) -> boolean())) -> boolean().
...
-spec all(iterator(BWJ), fun((BWJ) -> boolean())) -> boolean().
all(Iterator, Predicate) ->
    _pipe = erlang:element(2, Iterator),
    do_all(_pipe, Predicate).

-spec update_group_with(BWL) -> fun((gleam@option:option(list(BWL))) -> list(BWL)).
...
-spec group_updater(fun((BWP) -> BWQ)) -> fun((gleam@map:map_(BWQ, list(BWP)), BWP) -> gleam@map:map_(BWQ, list(BWP))).
group_updater(F) ->
    fun(Groups, Elem) -> _pipe = Groups,
        gleam@map:update(_pipe, F(Elem), update_group_with(Elem)) end.

-spec group(iterator(BWX), fun((BWX) -> BWZ)) -> gleam@map:map_(BWZ, list(BWX)).
group(Iterator, Key) ->
    _pipe = Iterator,
...
-spec reduce(iterator(BXD), fun((BXD, BXD) -> BXD)) -> {ok, BXD} | {error, nil}.
reduce(Iterator, F) ->
    case (erlang:element(2, Iterator))() of
        stop ->
            {error, nil};
...
-spec last(iterator(BXH)) -> {ok, BXH} | {error, nil}.
last(Iterator) ->
    _pipe = Iterator,
...
-spec once(fun(() -> BXN)) -> iterator(BXN).
once(F) ->
...
-spec single(BXP) -> iterator(BXP).
single(Elem) ->
    once(fun() -> Elem end).

-spec do_interleave(fun(() -> action(BXR)), fun(() -> action(BXR))) -> action(BXR).
...
-spec interleave(iterator(BXV), iterator(BXV)) -> iterator(BXV).
...
-spec do_fold_until(
    fun(() -> action(BXZ)),
    fun((BYB, BXZ) -> gleam@list:continue_or_stop(BYB)),
    BYB
) -> BYB.
...
-spec fold_until(
    iterator(BYD),
    BYF,
    fun((BYF, BYD) -> gleam@list:continue_or_stop(BYF))
) -> BYF.
...
-spec try_fold(iterator(BYP), BYR, fun((BYR, BYP) -> {ok, BYR} | {error, BYS})) -> {ok,
        BYR} |
    {error, BYS}.
...
-spec first(iterator(BYX)) -> {ok, BYX} | {error, nil}.
first(Iterator) ->
    case (erlang:element(2, Iterator))() of
        stop ->
            {error, nil};
...
-spec at(iterator(BZB), integer()) -> {ok, BZB} | {error, nil}.
at(Iterator, Index) ->
    _pipe = Iterator,
...
-spec each(iterator(BZJ), fun((BZJ) -> any())) -> nil.
each(Iterator, F) ->
    _pipe = Iterator,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).

-spec yield(BZM, fun(() -> iterator(BZM))) -> iterator(BZM).
yield(Element, Next) ->
